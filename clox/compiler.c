...
static void emitDword(uint8_t byte1, uint8_t byte2, uint8_t byte3, uint8_t byte4) {
  emitByte(byte1);
  emitByte(byte2);
  emitByte(byte3);
  emitByte(byte4);
...
static void namedVariable(Token name, bool canAssign) {
  ...
  if (canAssign && match(TOKEN_EQUAL)) {
    ...
    expression();
    emitDword(setOp, arg & 0xff, arg >> 8 & 0xff, arg >> 16 & 0xff);
  } else {
    emitDword(getOp, arg & 0xff, arg >> 8 & 0xff, arg >> 16 & 0xff);
  }
}
...
static void defineVariable(int global, bool mutable) {
  ...
  emitDword(OP_DEFINE_GLOBAL, global & 0xff, global >> 8 & 0xff, global >> 16 & 0xff);
}
