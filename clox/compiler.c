...
static void emitReturn() {
  if (current->type == TYPE_INITIALIZER) {
    emitLongInstruction(OP_GET_LOCAL, 0, 0, 0);
  } else {
    emitByte(OP_NIL);
  }
...
static void initCompiler(Compiler* compiler, FunctionType type) {
  ...
  current = compiler;
  if (type != TYPE_SCRIPT) {
    current->function->name = stringLiteral(parser.previous.start,
                                            parser.previous.length);
  }

  Token name;
  name.type = TOKEN_IDENTIFIER;
  name.line = parser.current.line;
  ...
  addLocal(name);
  current->locals[current->localCount - 1].depth = 0;
}
...
static int resolveLocal(Compiler* compiler, Token* name) {
  Value identifier = OBJ_VAL(stringLiteral(name->start, name->length));
  Value value;
  if (tableGet(&compiler->localTable, identifier, &value)) {
...
static void dot(bool canAssign) {
  ...
  if (canAssign && match(TOKEN_EQUAL)) {
    ...
    uint8_t argCount = argumentList();
    emitLongInstruction(OP_INVOKE, name & 0xff, name >> 8 & 0xff, name >> 16 & 0xff);
    emitByte(argCount);
  } else {
...
static void namedVariable(Token name, bool canAssign) {
  ...
  if (canAssign && match(TOKEN_EQUAL)) {
    ...
    if (setOp == OP_SET_LOCAL) {
      ...
      int index = arg;
      while (!compiler->upvalues[index].isLocal) {
        index = compiler->upvalues[index].index;
        compiler = compiler->enclosing;
      }
      local = &compiler->enclosing->locals[compiler->upvalues[index].index];
    }
...
static void super_(bool canAssign) {
  ...
  if (match(TOKEN_LEFT_PAREN)) {
    ...
    emitLongInstruction(OP_SUPER_INVOKE, name & 0xff, name >> 8 & 0xff, name >> 16 & 0xff);
    emitByte(argCount);
  } else {
    namedVariable(syntheticToken("super"), false);
    emitLongInstruction(OP_GET_SUPER, name & 0xff, name >> 8 & 0xff, name >> 16 & 0xff);
  }
}
...
static void method() {
  ...
  int constant = addConstant(currentChunk(), OBJ_VAL(stringLiteral(parser.previous.start, parser.previous.length)));
  ...
  function(type);
  emitLongInstruction(OP_METHOD, constant & 0xff, constant >> 8 & 0xff, constant >> 16 & 0xff);
}
